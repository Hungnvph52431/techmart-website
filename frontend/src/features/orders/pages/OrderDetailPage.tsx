...
import {
  ArrowLeft, RefreshCcw, Package, MapPin, CreditCard,
  FileText, Clock, CheckCircle2, Star, X, AlertCircle, RotateCcw,
            ...
            return (
              <div key={item.orderDetailId} className="space-y-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className={`ml-2 text-sm font-semibold ${rating ? 'text-yellow-600' : 'text-slate-300'}`}>
...
const ReturnModal = ({
  ...
  const [selected, setSelected] = useState<Record<number, { checked: boolean; quantity: number }>>(
    () => Object.fromEntries(items.map(it => [it.orderDetailId, { checked: true, quantity: it.quantity ?? 1 }]))
  );
  const [submitting, setSubmitting] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    if (returnItems.length === 0) { toast.error('Vui lòng chọn ít nhất một sản phẩm'); return; }
    try {
      setSubmitting(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white flex items-center justify-between px-6 py-4 border-b border-slate-100 rounded-t-3xl z-10">
          <div>
            <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
              <RotateCcw size={16} className="text-orange-500" /> Yêu cầu hoàn/trả hàng
            </h3>
            <p className="text-xs text-slate-400 mt-0.5">Chọn sản phẩm và lý do hoàn trả</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-xl transition-colors">
            <X size={17} className="text-slate-400" />
          </button>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {/* Lý do */}
          <div>
            <p className="text-sm font-bold text-slate-700 mb-2">Lý do hoàn trả <span className="text-rose-500">*</span></p>
            <div className="space-y-2">
              {RETURN_REASONS.map(r => (
                <label key={r} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border-2 cursor-pointer transition-colors ${reason === r ? 'border-orange-400 bg-orange-50' : 'border-slate-100 hover:border-slate-200'}`}>
                  <input type="radio" name="return-reason" value={r} checked={reason === r}
                    onChange={() => setReason(r)} className="w-4 h-4 accent-orange-500" />
          ...
          {/* Ghi chú thêm */}
          <div>
            <p className="text-sm font-bold text-slate-700 mb-2">Ghi chú thêm <span className="text-slate-400 font-normal">(không bắt buộc)</span></p>
...
export const OrderDetailPage = () => {
  ...
  useEffect(() => { void loadData(); }, [orderId]);
  ...
  const handleConfirmDelivered = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || 'Không thể xác nhận đã nhận hàng');
    } finally {
      setSubmitting(null);
    }
  };
    ...
    return (
      <div className="container mx-auto px-4 py-12">
        <div className="rounded-2xl border border-dashed border-gray-300 bg-white px-6 py-16 text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Không tìm thấy đơn hàng</h2>
          <p className="mt-2 text-sm text-gray-500">Đơn hàng không tồn tại hoặc bạn không có quyền.</p>
          <Link to="/orders"
  ...
  const orderDate   = order.orderDate  ?? order.created_at ?? order.createdAt ?? '';
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-8 space-y-6 max-w-5xl">
        ...
        <div className="flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          <div>
            <Link to="/orders" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-blue-600 transition-colors mb-3">
              <ArrowLeft size={16} /> Quay lại danh sách đơn hàng
            </Link>
            <div className="flex flex-wrap items-center gap-3">
              <h2 className="text-2xl font-black text-gray-900 uppercase italic">{orderCode}</h2>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {canCancel && (
              <button onClick={() => setShowCancelForm(p => !p)}
            ...
            {canConfirmReceived && (
              <button onClick={handleConfirmDelivered}
                ...
                {submitting === 'confirm-delivered' ? 'Đang xác nhận...' : 'Đã nhận hàng'}
              </button>
            )}
        ...
        {showCancelForm && canCancel && (
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5">
            <h3 className="text-base font-black text-rose-900 uppercase">Yêu cầu hủy đơn</h3>
            <textarea value={cancelReason} onChange={e => setCancelReason(e.target.value)}
              rows={3} placeholder="Nhập lý do hủy đơn..."
            ...
            <div className="mt-3 flex gap-3">
              <button onClick={handleCancelOrder} disabled={submitting === 'cancel'}
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_320px]">
          <div className="space-y-6">
                  ...
                  return (
                    <div key={item.orderDetailId ?? idx}
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="font-black text-gray-900">{formatCurrency(sub)}</p>
                        <p className="text-xs text-gray-400 mt-1">{formatCurrency(price)}/sp</p>
                      </div>
                    </div>
                  );
                })}
                    ...
                    <div key={event.orderEventId ?? idx} className="flex gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-gray-900">
                          {event.eventType ?? event.type ?? 'Cập nhật trạng thái'}
                        </p>
            ...
            {returns.length > 0 && (
              <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                <h3 className="text-base font-black text-gray-900 uppercase italic flex items-center gap-2 mb-5">
                  <RotateCcw size={18} className="text-orange-500" /> Yêu cầu hoàn/trả hàng
                </h3>
                <div className="space-y-4">
                  {returns.map((ret) => {
                    const RSTATUS: Record<string, { label: string; style: string }> = {
                      requested: { label: 'Chờ duyệt', style: 'bg-amber-100 text-amber-800' },
                      approved:  { label: 'Đã duyệt', style: 'bg-sky-100 text-sky-800' },
                      rejected:  { label: 'Từ chối', style: 'bg-rose-100 text-rose-800' },
                      received:  { label: 'Đã nhận hàng', style: 'bg-violet-100 text-violet-800' },
                      refunded:  { label: 'Đã hoàn tiền', style: 'bg-emerald-100 text-emerald-800' },
                      closed:    { label: 'Đã đóng', style: 'bg-gray-100 text-gray-600' },
                    };
                    const cfg = RSTATUS[ret.status] ?? RSTATUS.requested;
                    return (
                      <div key={ret.orderReturnId} className="rounded-xl border border-gray-100 p-4 space-y-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div>
                            <p className="font-bold text-gray-800 text-sm">{ret.requestCode}</p>
                            <p className="text-xs text-gray-400 mt-0.5">
                              Yêu cầu lúc {ret.requestedAt ? formatDateTime(ret.requestedAt) : '—'}
                            </p>
                          </div>
                          <span className={`px-3 py-1 rounded-full text-xs font-black uppercase ${cfg.style}`}>
                            {cfg.label}
                          </span>
                        </div>
                        ...
                        {ret.customerNote && (
                          <p className="text-xs text-gray-500 italic">"{ret.customerNote}"</p>
                        )}
                        {/* Ảnh bằng chứng */}
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-gray-400 font-bold pt-1 border-t border-gray-50">
                          {ret.approvedAt && <span>Duyệt: {formatDateTime(ret.approvedAt)}</span>}
                          {ret.rejectedAt && <span className="text-rose-400">Từ chối: {formatDateTime(ret.rejectedAt)}</span>}
                          {ret.receivedAt && <span>Nhận hàng: {formatDateTime(ret.receivedAt)}</span>}
                          {ret.refundedAt && <span className="text-emerald-500">Hoàn tiền: {formatDateTime(ret.refundedAt)}</span>}
                          {ret.closedAt && <span>Đóng: {formatDateTime(ret.closedAt)}</span>}
                        </div>
                      </div>
                    );
                  })}
          ...
          <aside className="space-y-6">
            <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-2 text-sm text-gray-600">
                <p><span className="font-bold text-gray-900">Người nhận:</span>{' '}{shipping.name ?? shipping.receiverName ?? '—'}</p>
                <p><span className="font-bold text-gray-900">SĐT:</span>{' '}{shipping.phone ?? shipping.receiverPhone ?? '—'}</p>
                <p><span className="font-bold text-gray-900">Địa chỉ:</span>{' '}{shipping.fullAddress ?? shipping.address ?? '—'}</p>
              </div>
            </section>
            ...
            {(customerNote || cancelReason_) && (
              <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  {customerNote && <p><span className="font-bold text-gray-900">Ghi chú:</span> {customerNote}</p>}
                  {cancelReason_ && <p><span className="font-bold text-rose-700">Lý do hủy:</span> {cancelReason_}</p>}
                </div>
              </section>
            )}
          </aside>
