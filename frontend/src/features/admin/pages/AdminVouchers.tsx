...
export const AdminVouchers = () => {
  ...
  // 1. State form khớp hoàn toàn với cấu trúc DB
  ...
  // 2. Hàm kiểm tra dữ liệu đầu vào (Chặn số âm, bỏ trống)
  const validateForm = () => {
    ...
    if (formData.usage_limit <= 0) return "Giới hạn sử dụng phải lớn hơn 0!";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payloadToSubmit = {
        ...formData,
        // Ép về 0 nếu là tiền mặt để MySQL không lỗi
      ...
      setErrorMsg("Lỗi từ Server. Vui lòng kiểm tra lại kết nối!");
    }
  };
                  ...
                  <tr key={v.coupon_id} className="hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="p-5 font-bold text-gray-800">
                      ...
                      {v.discount_type === 'percentage' && v.max_discount_amount && (
                        <div className="text-[10px] text-gray-400 font-normal">Tối đa: {Number(v.max_discount_amount).toLocaleString()}đ</div>
                      )}
                    </td>
                    <td className="p-5">
                    ...
                    {/* Cột Trạng thái: Tự động hết hạn */}
                    <td className="p-5 text-center">
                      {v.valid_to && new Date(v.valid_to).getTime() < new Date().getTime() ? (
                        <span className="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase bg-gray-200 text-gray-500 border border-gray-300">
                          Đã hết hạn
                        </span>
                      ) : v.is_active ? (
                        <span className="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase bg-green-100 text-green-700 border border-green-200">
                          Đang chạy
                        </span>
                      ) : (
                        <span className="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase bg-red-100 text-red-600 border border-red-200">
                          Đã dừng
                        </span>
                      )}
                    </td>

                    {/* Cột Thao tác: Nút xóa gọn gàng */}
                    <td className="p-5 text-center">
                      <button 
        ...
        <div className="fixed inset-0 bg-blue-900/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-[32px] w-full max-w-2xl shadow-2xl my-8 animate-in zoom-in duration-200">
            <div className="px-8 py-6 border-b flex justify-between items-center bg-gray-50/50 rounded-t-[32px]">
              <h2 className="text-2xl font-black text-gray-800">THIẾT LẬP MÃ GIẢM GIÁ</h2>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase mb-2">Mã Code *</label>
                  <input className="w-full bg-gray-50 border-2 border-gray-100 rounded-xl px-4 py-3 font-bold uppercase focus:border-blue-500 outline-none" value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} placeholder="VD: HELLO2026" />
                </div>
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase mb-2">Trạng thái</label>
                  <select className="w-full bg-gray-50 border-2 border-gray-100 rounded-xl px-4 py-3 font-bold focus:border-blue-500 outline-none" value={formData.is_active} onChange={e => setFormData({...formData, is_active: Number(e.target.value)})}>
              ...
              <div>
                ...
                <input className="w-full bg-gray-50 border-2 border-gray-100 rounded-xl px-4 py-3 font-medium focus:border-blue-500 outline-none" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Chi tiết chương trình khuyến mãi..." />
              </div>

              <div className="grid grid-cols-2 gap-6 p-5 bg-blue-50/50 rounded-2xl border border-blue-100">
                <div>
                  <label className="block text-xs font-black text-blue-800 uppercase mb-2">Loại Giảm</label>
                ...
                <div>
                  <label className="block text-xs font-black text-blue-800 uppercase mb-2">Giá trị giảm *</label>
                ...
                {formData.discount_type === 'percentage' && (
                  <div className="col-span-2">
                    <label className="block text-xs font-black text-blue-800 uppercase mb-2">Giảm tối đa (VNĐ)</label>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase mb-2">Đơn từ (đ) *</label>
                ...
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase mb-2">Tổng lượt *</label>
                ...
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase mb-2">Lượt/User *</label>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase mb-2">Từ ngày *</label>
                ...
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase mb-2">Đến ngày *</label>
