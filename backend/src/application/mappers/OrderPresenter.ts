...
const EVENT_LABELS: Record<OrderEvent['eventType'], string> = {
  ...
  return_inspected: 'Đã kiểm tra hàng hoàn trả',
...
const toReturn = (item: OrderReturn, actor: 'admin' | 'customer' | 'guest') => ({
  ...
  inspectedBy: actor === 'admin' ? item.inspectedBy : undefined,
  inspectionNote: item.inspectionNote,
  inspectionEvidenceImages: item.inspectionEvidenceImages,
  ...
  items: (item.items || []).map((returnItem) => ({
    ...
    inspectionResult: returnItem.inspectionResult,
    inspectionNote: returnItem.inspectionNote,
