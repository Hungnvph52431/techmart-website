...
export interface IReviewRepository {
  ...
  findProductReviewByIdForGuest(reviewId: number, orderId: number): Promise<ProductReview | null>;
  findReturnLinkedOrderDetailIds(
    orderId: number,
    userId: number
  ): Promise<Map<number, number>>;
  findReturnLinkedOrderDetailIdsForOrder(orderId: number): Promise<Map<number, number>>;
