...
CREATE TABLE users (
    ...
    phone VARCHAR(20),
    role ENUM('customer', 'admin', 'staff', 'shipper') DEFAULT 'customer',
...
CREATE TABLE order_events (
    ...
    actor_user_id INT,
    actor_role ENUM('customer', 'admin', 'staff', 'shipper', 'system'),
    note TEXT,
    metadata JSON,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (order_id) REFERENCES orders(order_id) ON DELETE CASCADE,
...
CREATE TABLE order_returns (
    ...
    status ENUM('requested', 'approved', 'rejected', 'received', 'inspected', 'refunded', 'closed', 'cancelled') DEFAULT 'requested',
    ...
    inspection_evidence_images JSON NULL,
...
CREATE TABLE order_return_items (
    ...
    inspection_result ENUM('good', 'defective', 'damaged_by_customer') NULL,
    inspection_note TEXT NULL,
    refund_amount DECIMAL(12,2) NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- ==================================================
-- BẢNG USER_POINTS_LOG - Lịch sử cộng/trừ điểm tích lũy
-- ==================================================
